...
class ClipiSupabaseUser extends BaseAuthUser {
  ClipiSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> clipiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ClipiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
